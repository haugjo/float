...
class TestEFS(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(self.efs.weights.shape, (self.efs.n_total_features,), msg='attribute weights is initialized correctly')
        self.assertIsInstance(self.efs.weights_history, list, msg='attribute weights_history is initialized correctly')
        self.assertIsInstance(self.efs.selected_features_history, list, msg='attribute selected_features_history is initialized correctly')
        self.assertIsInstance(self.efs.selected_features, list, msg='attribute selected_features is initialized correctly')
        self.assertEqual(self.efs._scale_warning_issued, False, msg='attribute _scale_warning_issued is initialized correctly')
...
def test_weight_features(self):
    ...
    self.assertEqual(weights.shape, self.efs.weights.shape, msg='weight_features() preserves the shape of attribute weights')
...
def test_select_features(self):
    ...
    self.assertEqual(len(self.efs.weights_history), 1, msg='select_features() sets the attribute weights_history correctly')
    self.assertEqual(len(self.efs.weights_history[0]), self.efs.n_total_features, msg='select_features() sets the attribute weights_history correctly')
    self.assertEqual(len(self.efs.selected_features_history), 1, msg='select_features() sets the attribute selected_features_history correctly')
    self.assertEqual(len(self.efs.selected_features_history[0]), self.efs.n_selected_features, msg='select_features() sets the attribute selected_features_history correctly')
