...
class TestHoldoutPipeline(unittest.TestCase):
    ...
    def test_run(self):
        ...
        self.assertIsInstance(self.holdout_pipeline.predictors[0].model.classifier.coef_, np.ndarray, msg='run() sets the classifier\'s weights')
        self.assertEqual(len(self.holdout_pipeline.prediction_evaluators[0].training_comp_times), self.holdout_pipeline.time_step, msg='run() adds a prediction training computation time for each time step')
        self.assertEqual(len(self.holdout_pipeline.prediction_evaluators[0].result['zero_one_loss']['measures']), int(np.ceil(self.holdout_pipeline.time_step / self.holdout_pipeline.test_interval)), msg='run() adds a prediction evaluation measure every "test_interval" time steps')
