...
class TestHoldoutPipeline(unittest.TestCase):
    ...
    def test_init(self):
        ...
        for pred in predictors:
            holdout_pipeline = HoldoutPipeline(data_loader=self.data_loader,
                                               predictor=pred,
                                               ...
                                               feature_selection_evaluator=copy.deepcopy(self.feature_selection_evaluator),
            ...
            self.assertEqual(holdout_pipeline.start_time, 0, msg='attribute start_time is initialized correctly')
            self.assertEqual(holdout_pipeline.time_step, 0, msg='attribute time_step is initialized correctly')
            self.assertEqual(holdout_pipeline.n_total, 0, msg='attribute n_total is initialized correctly')

            if isinstance(pred, list):
                self.assertEqual(len(holdout_pipeline.predictors), len(pred),
                                 msg='n predictors have been correctly initialized.')
                self.assertEqual(len(holdout_pipeline.prediction_evaluators), len(pred),
                                 msg='n prediction evaluators have been correctly initialized.')
            else:
                self.assertEqual(len(holdout_pipeline.predictors), 1,
                                 msg='n predictors have been correctly initialized.')
                self.assertEqual(len(holdout_pipeline.prediction_evaluators), 1,
                                 msg='n prediction evaluators have been correctly initialized.')

        with self.assertRaises(AttributeError, msg="AttributeError when passing no valid DataLoader object."):
            HoldoutPipeline(data_loader=None,
        ...
        with self.assertRaises(AttributeError,
                               msg='AttributeError when failing to provide a PredictorEvaluator object along the Predictor.'):
            HoldoutPipeline(self.data_loader,
                            predictor=SkmultiflowClassifier(PerceptronMask(),
                                                            classes=self.data_loader.stream.target_values),
        ...
        with self.assertRaises(AttributeError,
                               msg='AttributeError when failing to provide a ChangeDetectionEvaluator along the ChangeDetector.'):
            HoldoutPipeline(self.data_loader,
                            predictor=SkmultiflowClassifier(PerceptronMask(),
                                                            classes=self.data_loader.stream.target_values),
                            prediction_evaluator=copy.deepcopy(self.prediction_evaluator),
        ...
        with self.assertRaises(AttributeError,
                               msg='AttributeError when failing to provide a FeatureSelectionEvaluator along the FeatureSelection object.'):
            HoldoutPipeline(self.data_loader,
                            predictor=SkmultiflowClassifier(PerceptronMask(),
                                                            classes=self.data_loader.stream.target_values),
                            prediction_evaluator=copy.deepcopy(self.prediction_evaluator),
        ...
        with self.assertRaises(AttributeError,
                               msg='AttributeError when a multiclass dataset is used but the Feature Selector'
                                   'does not support multiclass targets'):
            data_loader = DataLoader(path='../datasets/gas.csv', target_col=-1)
            HoldoutPipeline(data_loader=data_loader,
                            predictor=SkmultiflowClassifier(PerceptronMask(),
                                                            classes=self.data_loader.stream.target_values),
                            prediction_evaluator=copy.deepcopy(self.prediction_evaluator),
                            feature_selector=OFS(n_total_features=data_loader.stream.n_features,
                                                 n_selected_features=10))
    ...
    def test_run(self):
        for batch_size in range(1, 100, 10):
            ...
            pipeline = HoldoutPipeline(data_loader=self.data_loader,
                                       predictor=SkmultiflowClassifier(PerceptronMask(),
                                                                       classes=self.data_loader.stream.target_values),
                                       prediction_evaluator=copy.deepcopy(self.prediction_evaluator),
                                       feature_selector=OFS(
            ...
            pipeline.run()
            self.assertIsInstance(pipeline.predictors[0].model.classifier.coef_, np.ndarray,
