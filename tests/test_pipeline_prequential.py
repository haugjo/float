...
class TestPrequentialPipeline(unittest.TestCase):
    def __init__(self, method_name):
        ...
        self.known_drifts = [round(self.data_loader.stream.n_samples * 0.2), round(self.data_loader.stream.n_samples * 0.4),
    ...
    def test_init(self):
        ...
        for pred in predictors:
            prequential_pipeline = PrequentialPipeline(data_loader=copy.deepcopy(self.data_loader),
                                                       predictor=pred,
            ...
            self.assertEqual(prequential_pipeline.start_time, 0, msg='attribute start_time is initialized correctly')
            self.assertEqual(prequential_pipeline.time_step, 0, msg='attribute time_step is initialized correctly')
            self.assertEqual(prequential_pipeline.n_total, 0, msg='attribute n_total is initialized correctly')

            if isinstance(pred, list):
                self.assertEqual(len(prequential_pipeline.predictors), len(pred),
                                 msg='n predictors have been correctly initialized.')
                self.assertEqual(len(prequential_pipeline.prediction_evaluators), len(pred),
                                 msg='n prediction evaluators have been correctly initialized.')
            else:
                self.assertEqual(len(prequential_pipeline.predictors), 1,
                                 msg='n predictors have been correctly initialized.')
                self.assertEqual(len(prequential_pipeline.prediction_evaluators), 1,
                                 msg='n prediction evaluators have been correctly initialized.')

        with self.assertRaises(AttributeError, msg="AttributeError when passing no valid DataLoader object."):
            PrequentialPipeline(data_loader=None,
    ...
    def test_run(self):
        for batch_size in range(1, 100, 10):
            ...
            pipeline = PrequentialPipeline(data_loader=copy.deepcopy(self.data_loader),
                                           predictor=SkmultiflowClassifier(PerceptronMask(),
                                                                           classes=self.data_loader.stream.target_values),
                                           prediction_evaluator=copy.deepcopy(
            ...
            self.assertEqual(len(pipeline.prediction_evaluators[0].result['zero_one_loss']['measures']), pipeline.time_step,
            ...
            self.assertEqual(len(pipeline.feature_selection_evaluator.result['nogueira_stability']['measures']), pipeline.time_step,
