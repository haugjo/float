...
class TestRiverClassifier(unittest.TestCase):
    ...
    def test_init(self):
        self.assertEqual(self.river_classifier.reset_after_drift, False,
        ...
        self.assertIsInstance(self.river_classifier.feature_names, list,
    ...
    def test_partial_fit(self):
        ...
        self.assertEqual(n_iterations + 1, self.river_classifier.model.optimizer.n_iterations,
                         msg='partial_fit() updates the optimizer of the underlying model')
    ...
    def test_predict(self):
        ...
        self.assertTrue((np.isin(self.river_classifier.predict(X), self.data_loader.stream.target_values)).all(),
    ...
    def test_predict_prob(self):
        ...
        self.river_classifier.partial_fit(X, y)
        X, y = self.data_loader.get_data(10)
        self.assertEqual((y.shape[0], 2), self.river_classifier.predict_proba(X).shape,
                         msg='predict_proba() returns the correct shape')
        proba = self.river_classifier.predict_proba(X)
        self.assertTrue(((proba >= 0).all() | (proba <= 1).all()).all(),
                        msg='predict_proba() only returns values between 0 and 1')

    def test_reset(self):
        X, y = self.data_loader.get_data(10)
        self.river_classifier.partial_fit(X, y)
        self.river_classifier.reset()
        self.assertEqual(0, self.river_classifier.model.optimizer.n_iterations,
                         msg='model has been reset, i.e. trained in 0 iterations.')
        self.assertFalse(self.river_classifier.has_been_trained, msg='classifier has not been trained')
