...
class TestSpiderChart(unittest.TestCase):
    def __init__(self, method_name):
        ...
        self.sample_measures = [list(np.random.rand(3)) for _ in range(4)]

    def test_spider_chart(self):
        ax = fvis.spider_chart(measures=self.sample_measures,
                               measure_names=[f'metric_{i}' for i in range(4)],
        ...
        self.assertEqual([text.get_text() for text in ax.get_yticklabels()], ['', '0.2', '0.4', '0.6', '0.8', ''],
        ...
        self.assertEqual([self.sample_measures[0][0], 1 - self.sample_measures[1][0],
                          self.sample_measures[2][0] / 2, 1 - (self.sample_measures[3][0] / 2)], [x[1] for x in ax.patches[0].xy][:-1],
